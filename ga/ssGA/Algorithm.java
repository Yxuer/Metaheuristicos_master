...
public class Algorithm
{
  ...
  private int seed;                   // Random seed for experiment replicability
  private  Individual aux_indiv;  // Internal auxiliar individual being computed


  // CONSTRUCTOR
  public Algorithm(Problem p, int popsize, int gn, int gl, double pc, double pm, int seed)
  throws Exception
  {
    this.gene_number   = gn;
    this.gene_length   = gl;
    ...
    this.pm            = pm;
    this.problem       = p;                     
    this.pop = new Population(popsize,chrom_length,seed);// Create initial population
    this.r             = new Random();
    r.setSeed(seed);
    this.seed = seed;
    this.aux_indiv     = new Individual(chrom_length, seed);
